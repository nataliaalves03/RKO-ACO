...
{                          
    ...
    const char *all_algorithms[TOTAL_MH] = {
        "ACO","BRKGA", "SA", "GRASP", "ILS", "VNS", "PSO", "GA", "LNS", "BRKGA-CS", "MultiStart", "IPR"
    };

    // Declare as funções com a assinatura correta
    void ACO(const TRunData &, const TProblemData &);
    ...
    void (*all_functions[TOTAL_MH])(const TRunData &, const TProblemData &) = {
        ACO, BRKGA, SA, GRASP, ILS, VNS, PSO, GA, LNS, BRKGA_CS, MultiStart, IPR
    };
    ...
    TTT_print(foBest, timeBest, nameInstance, NUM_MH);
