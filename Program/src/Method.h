...
void CreatePoolSolutions(const TProblemData &data, const int sizePool)
{
    #pragma omp critical
    {
        for (int i = 0; i < sizePool; i++){
            ...
            CreateSolutionSemiGreedy(pool[i], data);
...
*************************************************************************************/
void TTT(int foBest, int target = 420189)
{
    /*if (floor(foBest) <= target)
        stop_execution.store(true);*/
}

void TTT_reset(TRunData &runData)
{
    //runData.MAXTIME = 0;
}

void TTT_print(int foBest, float best_time, char* nameInstance, int num_mh, int target = 420189)
{
    /*// TTT plot
    char name[256];
    snprintf(name, sizeof(name), "../Results/TTT/TTT_1thr_VNS.txt", nameInstance);
    //snprintf(name, sizeof(name), "../Results/TTT/TTT_25-11_1thr_5perc.txt", nameInstance);
    FILE *tttFile;
    tttFile = fopen(name, "a");
    if (tttFile == NULL) {
        // Try to create the file
        FILE *create = fopen(name, "w");
        if (create != NULL) fclose(create);

        // Try opening again for append
        tttFile = fopen(name, "a");
        if (tttFile == NULL) {
    ...
    if (floor(foBest) <= target){
        fprintf(tttFile, "%lf\t%d\t%s\t%d\n", best_time, target, nameInstance, num_mh);
        //fprintf(tttFile, "%lf\n", best_time);
    }else{
        fprintf(tttFile, "TIMEOUT\n");
    }
    fclose(tttFile);*/
}
...
*************************************************************************************/
void UpdatePoolSolutions(TSol s, const char* mh, const int debug, int maxPoolSize = 10)
{
    #pragma omp critical
    {
        // If the pool is full and the new solution is not better than the worst, skip
        if ((int)pool.size() < maxPoolSize || pool.back().ofv - s.ofv >= 1) {
...
void NelderMeadSearch(TSol &x1, const TProblemData &data)
{
    ...
    // Ensure all rk vectors have correct size
    /*x1.rk.resize(data.n, 0.0);
    x2.rk.resize(data.n, 0.0);
    x3.rk.resize(data.n, 0.0);*/
